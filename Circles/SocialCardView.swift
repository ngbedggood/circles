            ...
            VStack {
                ...
                ZStack {
                    GeometryReader { geometry in
                        ...
                        let meY = center.y
                        let meX = center.x
                        let meScale: CGFloat = isMeSelected ? 3.0 : (someoneElseSelected ? 0.1 : 1.2)
                    ...
                    ForEach(Array(socialCard.friends.enumerated()), id: \.element.id) { index, friend in
                        ...
                        Circle()
                            ...
                            .padding(20)
                            .clipShape(Circle())
                            .font(selectedFriend?.id == friend.id ? .system(size: 6) : .system(size: 24))
                            .scaleEffect(scale)
                            .position(x: x, y: y)
                            .shadow(color: .black.opacity(0.2), radius: 4)
                            .onTapGesture {
...
#Preview {
    struct PreviewWrapper: View {
        @State private var socialCard = SocialCard(date: "24th June 2025", friends: [FriendColor(name: "Jack", color: .green, note: "I'm feeling great!"), FriendColor(name: "Greg", color: .teal, note: "I'm alright, just a bit tired!")])
        @State private var personalCard = PersonalCard(date: "24th June 2025", color: .teal, note: "I'm feeling a bit eh today... I'm alright, just a bit tired! Today might just be fine. This is a testtesttestetststs!!!!")
